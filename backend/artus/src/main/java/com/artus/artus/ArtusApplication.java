...
public class ArtusApplication implements CommandLineRunner{
	...
	public void run(String... strings) throws Exception {
		...
		jdbcTemplate.execute("create table if not exists Purchase(user_id int, artwork_id int, purchase_id int NOT NULL AUTO_INCREMENT, purchase_date date, price double, receipt BLOB, primary key (user_id, artwork_id, purchase_id), foreign key (artwork_id) references Artwork(artwork_id), foreign key (user_id) references User(user_id));");
