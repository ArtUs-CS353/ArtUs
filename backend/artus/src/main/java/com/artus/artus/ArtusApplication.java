...
public class ArtusApplication implements CommandLineRunner{
	...
	public void run(String... strings) throws Exception {
		...
		jdbcTemplate.execute("create table if not exists Bid(user_id int,auction_id int,bid_id int NOT NULL AUTO_INCREMENT,amount double,time_stamp datetime, status varchar(50), primary key (bid_id),foreign key (user_id) references User(user_id),foreign key (auction_id) references Auction(auction_id));");
