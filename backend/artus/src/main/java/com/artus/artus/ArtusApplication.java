...
public class ArtusApplication implements CommandLineRunner{
	...
	public void run(String... strings) throws Exception {
		...
		jdbcTemplate.execute("create table if not exists Event(artist_id int, event_id int NOT NULL AUTO_INCREMENT, meeting_link varchar(50), start_date datetime, end_date datetime, poster_URL varchar(50), status varchar(50), primary key (event_id),foreign key (artist_id) references Artist (user_id));");
		jdbcTemplate.execute("create table if not exists Exhibition(exhibition_id int NOT NULL AUTO_INCREMENT, exhibition_name varchar(50), start_date datetime, end_date datetime,poster_URL varchar(50), primary key (exhibition_id));");
		...
		try {
			...
			jdbcTemplate.execute("CREATE TRIGGER after_includes_insert AFTER INSERT ON Includes FOR EACH ROW BEGIN DECLARE artistUserID INT; SELECT artist_id INTO artistUserID FROM Artwork WHERE artwork_id = NEW.artwork_id; INSERT INTO Notification(user_id, type, content) VALUES (artistUserID, 'exhibition_add', CONCAT('Your artwork with ID ', NEW.artwork_id, ' has been added to a exhibition.')); END;");
			...
			jdbcTemplate.execute("CREATE TRIGGER auction_request_notification AFTER UPDATE ON Auction FOR EACH ROW BEGIN INSERT INTO Notification(user_id, type, content) VALUES (user_id, 'Auction Request', CONCAT('Your auction request has been ', status)); END;");
