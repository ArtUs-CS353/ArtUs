...
public class ArtusApplication implements CommandLineRunner{
	...
	public void run(String... strings) throws Exception {
		...
		jdbcTemplate.execute("create table if not exists Artwork (artwork_id INT NOT NULL AUTO_INCREMENT, artist_id INT, title VARCHAR(50), description VARCHAR(300), type VARCHAR(50), material VARCHAR(150), size VARCHAR(30), rarity VARCHAR(30), imageURL varchar(300), movement varchar(100), date DATE, is_featuring boolean,price NUMERIC(30), status varchar(20),availability varchar(20),favorite_count INT, primary key (artwork_id), foreign key (artist_id ) REFERENCES Artist(user_id));");
		...
		jdbcTemplate.execute("create table if not exists Event(user_id int, event_id int NOT NULL AUTO_INCREMENT, title varchar(50), meeting_link varchar(50), start_date datetime, end_date datetime, poster_URL varchar(300), status varchar(50), primary key (event_id),foreign key (user_id) references Artist (user_id));");
		...
		try {
			jdbcTemplate.execute("CREATE TRIGGER create_notification_after_bid_insert AFTER INSERT ON Bid FOR EACH ROW BEGIN DECLARE auction_id INT;  DECLARE user_id INT; SET auction_id = NEW.auction_id;  SET user_id = NEW.user_id; INSERT INTO Notification (user_id, type, content) SELECT DISTINCT user_id, 'Bid Placed', CONCAT('A new bid has been placed on auction ', auction_id) FROM Bid  WHERE auction_id = auction_id AND user_id <> user_id; END;\n");
			...
			jdbcTemplate.execute("CREATE TRIGGER update_artwork_status_after_purchase AFTER INSERT ON Purchase FOR EACH ROW BEGIN UPDATE Artwork SET availability = 'Sold', status = 'Sold' WHERE artwork_id = NEW.artwork_id; END;");
